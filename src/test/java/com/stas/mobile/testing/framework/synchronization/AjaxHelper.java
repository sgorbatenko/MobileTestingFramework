...
public class AjaxHelper
{
    ...
    public void waitForElementForNoTimeout(UIData element, boolean appear, int timeout)
    {
        String absoluteSelector = element.getAbsoluteSelector();
        waitForElementWithSelectorForNoTimeout(absoluteSelector, appear, timeout);
    }

    public void waitForElementWithSelectorForNoTimeout(String absoluteSelector, boolean appear, int timeout)
    {
        try
        {
            waitForElementAppearanceChangedForTimeout(absoluteSelector, appear, timeout);
        }
        catch (RuntimeException e)
        {
            System.err.println("Haven't found the element '" + absoluteSelector + "' - proceeding without exception");
        }
    }

    public void waitForElementWithSelectorAppearanceChange(String selector, boolean appear)
    {
        waitForElementWithSelectorAppearanceChangeForTimeout(selector, appear, 60);
    }

    public void waitForElementAppearanceChangedForTimeout(String selector, boolean appear, int timeout)
    {
        waitForElementWithSelectorAppearanceChangeForTimeout(selector, appear, timeout);
    }

    public void waitForElementWithSelectorAppearanceChangeForTimeout(final String selector, final boolean appear, int timeout)
    {
        this.logger.debug("Waiting for element to " + (appear ? "appear: " : "disappear: ") + selector);
        ...
        Function<WebDriver, Boolean> function = new Function()
        {
            ...
            {
                boolean condition = AjaxHelper.this.queryHelper.isElementPresent(selector);
                if (!appear)
                {
                    condition = !condition;
                }
        ...
        {
            throw new RuntimeException("Exception getting element: " + selector, e);
        }
    }
    ...
    public void waitForElementToAppear(String selector)
    {
        ...
        while (TestHelper.getCurrentGMT6Time() < end)
        {
            try
            {
                element = this.queryHelper.findElementByExtendedCss(selector);
                if (element == null)
                {
                    suspend(1000);
    ...
    public void waitForElementToAppear(BaseHtmlElement element)
    {
        ...
        while (TestHelper.getCurrentGMT6Time() < end)
        {
            try
            {
                ...
                {
                    suspend(2000);
                    this.logger.info("Waiting for " + element.getAbsoluteSelector() + " to appear");
                }
            ...
            catch (NoSuchElementException nse)
            {
                if (nseLogMessage)
                {
                    this.logger.debug("Element [" + element.getAbsoluteSelector()
        ...
        if (!element.isDisplayed().booleanValue())
        {
            Assert.fail("Element [" + element.getAbsoluteSelector() + "] was not found before timeout.");
        }
    }

    public void waitForElementToAppear(String selector, int waitTime, int retryCount, WebDriver driver)
    {
        int count = 0;
        while (!this.queryHelper.doesElementExist(selector))
        {
            scrollDown(driver);
            this.syncHelper.suspend(waitTime);
            scrollUp(driver);
            count++;
            if (count > retryCount)
            {
                Assert.fail(String.format("Element with selector [%s] not found before timeout.", new Object[]{selector}));
            }
        }
    }

    public void waitForElementToAppear(BaseHtmlElement element, int waitTime, int retryCount, WebDriver driver)
    {
        int count = 0;
        while (!element.exists())
        {
            ...
            if (count > retryCount)
            {
                Assert.fail(String.format("Element with selector [%s] not found before timeout.", new Object[]{element
    ...
    public void waitForElementToDisappear(String selector)
    {
        ...
        while (TestHelper.getCurrentGMT6Time() < end)
        {
            WebElement element = this.queryHelper.findElementByExtendedCss(selector);
            if (element != null)
            {
                suspend(100);
    ...
    {
        this.logger.debug("Dragging '" + whatSelector + "' to '" + whereSelector + "'");
        ...
        builder.dragAndDrop(this.queryHelper.findElementByExtendedCss(whatSelector), this.queryHelper
            .findElementByExtendedCss(whereSelector));
        builder.perform();
    }

    public void moveMouseTo(AbstractPageChunk element)
    {
        this.logger.debug("Moving the mouse explicitly to " + element
            .getAbsoluteSelector());
    ...
    {
        this.logger.debug("Firing 'click' explicitly " + element
            .getAbsoluteSelector());
        waitForElementAppearanceChanged(element, true);
        try
        {
            ((JavascriptExecutor) this.driver).executeScript("jQuery('" + element
                .getAbsoluteSelector() + "').click()", new Object[0]);
        }
        catch (WebDriverException e)
        {
            this.logger.error("Webdriver throws exeption" + element
                .getAbsoluteSelector() + "' - proceeding without exception");
        }
    }

    public void scrollToElement(String elementPath, WebDriver driver)
    {
        this.queryHelper = new WebElementQueryHelper(driver);
    ...
    public void scrollUp(WebDriver driver)
    {
        ...
        driver.findElement(By.tagName("body"))
            .sendKeys(new CharSequence[]{Keys.HOME});
    }
